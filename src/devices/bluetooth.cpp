...
void bluetooth::deviceDiscovered(const QBluetoothDeviceInfo &device) {
    ...
    if (searchDevices) {
        ...
        for (const QBluetoothDeviceInfo &b : qAsConst(devices)) {
            ...
            } else if ((b.name().toUpper().startsWith(QStringLiteral("SPERAX_RM01")) ||
                        b.name().toUpper().startsWith(QStringLiteral("SPERAX_RM-02")) ||
                        b.name().toUpper().startsWith(QStringLiteral("VIBROTECH1"))) &&
                       !speraXTreadmill && filter) {
                this->setLastBluetoothDevice(b);
                this->stopDiscovery();
